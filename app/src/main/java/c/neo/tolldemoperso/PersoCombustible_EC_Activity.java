...
import c.neo.secure.PersoSecure_Combustible_EC;

public class PersoCombustible_EC_Activity extends Activity {

	private final String TAG = PersoCombustible_EC_Activity.class.getSimpleName();
	EditText edittext;
	CheckBox subsidio;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_perso_combustible_ec);
	...
	private void write(Tag tag) throws IOException, FormatException {
		...
		edittext = (EditText)findViewById(R.id.folioTagID);
		cadena_grabar+=edittext.getText().toString().toUpperCase(Locale.getDefault())+"|";
		edittext = (EditText)findViewById(R.id.placa);
		cadena_grabar+=edittext.getText().toString().toUpperCase(Locale.getDefault())+"|";
		subsidio = (CheckBox)findViewById(R.id.subsidioID);
		cadena_grabar+=String.valueOf(subsidio.isChecked())+"|";
		edittext = (EditText)findViewById(R.id.galonesMesID);
		...
		if(cadena_grabar.isEmpty() ){
	        ...
	        PersoSecure_Combustible_EC perso = new PersoSecure_Combustible_EC();
	        String stringKey = perso.bin2hex(tag.getId())+"NeoGasEc";
	        try {
				key = perso.getkey(stringKey);
				keypbe = perso.getkey(stringKey);
				encryptedData = PersoSecure_Combustible_EC.encrypt(key,b);
	        ...
	        NdefRecord relayRecord = new NdefRecord(NdefRecord.TNF_MIME_MEDIA,
	                new String("app/com.neo.gas_ec").getBytes(Charset.forName("US-ASCII")),
	                null, encryptedData);
	        
	        NdefRecord[] records = { relayRecord, NdefRecord.createApplicationRecord("com.neo.gas_ec") };
