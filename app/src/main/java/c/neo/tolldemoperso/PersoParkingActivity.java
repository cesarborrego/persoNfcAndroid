...
public class PersoParkingActivity extends Activity {
	
	private final String TAG = PersoParkingActivity.class.getSimpleName();
	...
	private void write(Tag tag) throws IOException, FormatException {
        ...
        String stringKey = PersoSecureParking.bin2hex(tag.getId())+"NeoParking";
        try {
			key = PersoSecureParking.getkey(stringKey);
			keypbe = PersoSecureParking.getkey(stringKey);
			encryptedData = PersoSecureParking.encrypt(key,b);
			
			encryptedString = PersoSecureParking.bin2hex(encryptedData);
			Log.d(TAG, "PERSO::Key: " + PersoSecureParking.bin2hex(key));
			Log.d(TAG, "PERSO::KeyPBE: " + PersoSecureParking.bin2hex(keypbe));
        ...
        NdefRecord relayRecord = new NdefRecord(NdefRecord.TNF_MIME_MEDIA,
                new String("app/c.neo.parking").getBytes(Charset.forName("US-ASCII")),
                null, encryptedData);
        
        NdefRecord[] records = { relayRecord, NdefRecord.createApplicationRecord("c.neo.parking") };
