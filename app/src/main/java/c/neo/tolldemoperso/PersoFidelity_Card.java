...
public class PersoFidelity_Card extends Activity {

	private final String TAG = PersoFidelity_Card.class.getSimpleName();
	EditText edittext;
	CheckBox subsidio;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_perso_fidelitycard);
	...
	private void write(Tag tag) throws IOException, FormatException {
		...
		cadena_grabar+=edittext.getText().toString().toUpperCase(Locale.getDefault())+"|";
		edittext = (EditText)findViewById(R.id.balanceID);
		cadena_grabar+=edittext.getText().toString().toUpperCase(Locale.getDefault())+"|";
		edittext = (EditText)findViewById(R.id.pointsID);
		cadena_grabar+=edittext.getText().toString().toUpperCase(Locale.getDefault())+"|";
		edittext = (EditText)findViewById(R.id.creationDateID);
		cadena_grabar+=edittext.getText().toString().toUpperCase(Locale.getDefault())+"|";
		edittext = (EditText)findViewById(R.id.expirationDateID);
		cadena_grabar+=edittext.getText().toString().toUpperCase(Locale.getDefault())+"|";
		edittext = (EditText)findViewById(R.id.fuelTypeID);
		...
		if(cadena_grabar.isEmpty() ){
	        ...
	        PersoSecure_LoyaltyCard perso = new PersoSecure_LoyaltyCard();
	        String stringKey = perso.bin2hex(tag.getId())+"NeoShellCard";
	        try {
	        ...
	        NdefRecord relayRecord = new NdefRecord(NdefRecord.TNF_MIME_MEDIA,
	                new String("app/com.neology.loyaltycard").getBytes(Charset.forName("US-ASCII")),
	                null, encryptedData);
	        
	        NdefRecord[] records = { relayRecord, NdefRecord.createApplicationRecord("com.neology.loyaltycard") };
