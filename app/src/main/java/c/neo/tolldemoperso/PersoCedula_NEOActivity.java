...
public class PersoCedula_NEOActivity extends Activity {

	private final String TAG = PersoCedula_NEOActivity.class.getSimpleName();
	EditText edittext;
...
//		cadena_grabar+=edittext.getText()+"|";
//		EditText lugarEmision = (EditText)findViewById(R.id.lugar_emision);
//		lugarEmision.setEnabled(false);
//		cadena_grabar+=lugarEmision.getText()+"|4";
		
		String cadena_grabar = "";
		EditText matricula = (EditText)findViewById(R.id.matricula);
		cadena_grabar+=matricula.getText().toString().toUpperCase(Locale.getDefault())+"|";
		EditText cedula = (EditText)findViewById(R.id.poliza);
		cadena_grabar+=cedula.getText()+"|";
		EditText serie = (EditText)findViewById(R.id.serie);
		cadena_grabar+=serie.getText()+"|";
		EditText seccion = (EditText)findViewById(R.id.seccion);
		cadena_grabar+=seccion.getText()+"|";
		EditText fecha_emision = (EditText)findViewById(R.id.fecha_emision);
		cadena_grabar+=fecha_emision.getText()+"|";
		EditText fecha_vencimiento = (EditText)findViewById(R.id.fecha_vencimiento);
		cadena_grabar+=fecha_vencimiento.getText()+"|";
		EditText lugarEmision = (EditText)findViewById(R.id.lugar_emision);
		lugarEmision.setEnabled(false);
		cadena_grabar+=lugarEmision.getText()+"|4";
		...
		if(cadena_grabar.isEmpty() ){
	        ...
	        PersoSecure_NEO_Cedula perso = new PersoSecure_NEO_Cedula();
	        String stringKey = perso.bin2hex(tag.getId())+"CedulaNeov2";
	        try {
				key = perso.getkey(stringKey);
				keypbe = perso.getkey(stringKey);
				encryptedData = PersoSecure_NEO_Cedula.encrypt(key,b);
	        ...
	        NdefRecord relayRecord = new NdefRecord(NdefRecord.TNF_MIME_MEDIA,
	                new String("app/c.neo.cedula2").getBytes(Charset.forName("US-ASCII")),
	                null, encryptedData);
	        
	        NdefRecord[] records = { relayRecord, NdefRecord.createApplicationRecord("c.neo.cedula2") };
